...
- Scheduled refresh using Cron + a secured admin endpoint
...
- A  cron job triggers refresh automatically (Github Action).
...
### DB (Neon PostgreSQL)
![DB Demonstrations](/images/projects/btcexplainer/img_03.png)
...
## Cron Job Strategy ( Github action + Feature Flag)

### Why use Github action
Using Cron keeps production scheduling outside the application container:
...
-  Github action calls the admin refresh endpoint instead
